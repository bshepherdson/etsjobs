...
(defn- data-block
  ...
  [s buf typ]
  (let [id               (sr-id buf)
        {:keys [fields]} (get-in s [:structures typ])]
    (assoc-in
          s
      [:data id]
      (into {:type typ}
            (for [{:keys [name type]} fields]
              [name ((get value-handlers type) buf)])))))

(defn parse-sii [buf]
  ; Skip over the header
  (.position buf 8)
  (loop [s {:structures {} :data {}}]
    (let [typ (sr-int buf)]
      (if (zero? typ)
        (if-let [sdef (read-struct-block buf)]
          (recur (assoc-in s [:structures (:id sdef)] sdef))
          s)
        (recur (data-block s buf typ))))))
